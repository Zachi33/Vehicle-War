...
public class Box2dLightTest extends InputAdapter implements ApplicationListener {
	...
	public void render() {
		...
		if (showText) {
			...
			font.draw(batch,
					...
					+ Float.toString(czas) + "ns"+" thus fps equals:" +Float.toString((float)(1/(czas))) , 0, 20);
	...
	private final static int MAX_FPS = 30;//jeżeli kompputer jest super szybki to żeby nie liczył więcej niż 30 razy w ciągu sekundy bo za szybko będzie zapieprzać
	private final static int MIN_FPS = 15;//nie liczymy nic innego jeśli minęła sekunda  a my nie mamy 15 klatek
	public final static float TIME_STEP = 1f / MAX_FPS;//to jest czas który możemy wziąć przy max fps - nakrótszy czas
